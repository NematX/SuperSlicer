                ...
                for (auto [filament_id, filament_volume] : ps.filament_stats) {
                    ...
                    if (filament_preset) {
                        ...
                        if (items_printed == 0) {
                            ...
                            info_text += wxString::Format("\n%.4f", filament_volume * mm3_to_m);
                        } 
                        else {
                            new_label += "\n    - " + format_wxstr(_L("Color %1% at extruder %2%"), (items_printed+1), (filament_id + 1));
                            info_text += wxString::Format("\n%.4f (%.4f)", (filament_volume - total_length) * mm3_to_m, filament_volume * mm3_to_m);
                        }
                    }
                }
            }
                    ...
                    for (auto [filament_id, filament_volume] : ps.filament_stats) {
                        ...
                        if (filament_preset) {
                            ...
                            if (items_printed == 0) {
                                new_label += "\n    - " + format_wxstr(_L("Filament at extruder %1%"), filament_id + 1);
                                //new_label += from_u8((boost::format("\n    - %1% %2%") % _utf8(L("Color")) % ps.color_extruderid_to_used_filament.size()).str());
                                info_text += wxString::Format("\n%.4f", filament_volume * mm3_to_g);
                                if (spool_weight != 0.0)
                                    info_text += wxString::Format(" (%.4f)", filament_volume * mm3_to_g + spool_weight);
                            } else {
                                new_label += "\n    - " + format_wxstr(_L("Color %1% at extruder %2%"), (items_printed + 1), (filament_id + 1));
                                info_text += wxString::Format("\n%.4f", (filament_volume - total_length) * mm3_to_g);
                                if (spool_weight != 0.0)
                                    info_text += wxString::Format(" (%.4f)", (filament_volume - total_length) * mm3_to_g + spool_weight);
                            }
                        }
                    }
                    if (has_spool)
