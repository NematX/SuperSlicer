...
PageShp TabFilament::create_filament_overrides_page()
{
    ...
    for (const auto&[title, keys] : fff_filament_override_option_keys) {
        ...
        for (const std::string &opt_key : keys) {
            Option opt = optgroup->create_option_from_def(opt_key, extruder_idx);
	        ...
	        if(optgroup->use_custom_ctrl()) // fill group and category values just for options from Settings Tab
		        optgroup->register_to_search(opt_key, opt.opt, extruder_idx, true);
            optgroup->append_line(optgroup->create_single_option_line(opt));
        }
    }
