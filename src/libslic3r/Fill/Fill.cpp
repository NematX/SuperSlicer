            ...
            if (!expoly.contour.empty()) {
                ...
                if (surface_fill.params.fill_exactly && surface_fill.params.density == 1) {
                    ...
                    if(compute_volume.volume > 0) //can fail for thin regions
                        assert(surface_fill.params.flow.spacing_ratio() < 1 || compute_volume.volume >= area * surface_fill.params.layer_height * 0.999 || f->debug_verify_flow_mult >= 1.3 || area < std::max(1.,surface_fill.params.config->solid_infill_below_area.value) || area < std::max(1.,surface_fill.params.config->solid_infill_below_layer_area.value));
                }
#endif
            }
        }
